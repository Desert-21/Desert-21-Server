...
@SpringBootTest
class ArmyEnteringExecutorTest {
    ...
    @Autowired
    private ArmyEnteringExecutor tested;
    ...
    void executeHappyPath() throws NotAcceptableException {
        var results = tested.execute(
                List.of(new ArmyEnteringEvent(new Location(0, 0), new Army(10, 1, 4))),
                context
        );
        var newArmy = context.game().getFields()[0][0].getArmy();
        assertEquals(new Army(30, 4, 18), newArmy);
    ...
    void executeShouldThrowErrorOnNotFoundField() {
        var exception = assertThrows(NotAcceptableException.class, () -> {
            tested.execute(
                    List.of(
                            new ArmyEnteringEvent(new Location(0, 0), new Army(10, 1, 4)),
                            new ArmyEnteringEvent(new Location(-1, 99), new Army(10, 1, 4))
                    ),
                    context
            );
        });
